...
"""
These criteria functions help determine how to update the current constraints for the
phoneme selector. Based on the phonemes in the selected phoneme list, and the current
number of places, manners, or laryngeal features, they check which criteria are met.
If any are, then they return the corresponding features.
"""
...
def mannerCriteria(curr_num_manners: int, sel_phonemes: list[list]) -> list[int]:
    phoneme_bin = sel_phonemes[-1][1]
    curr_place = phoneme_bin % 32
    curr_manner = (phoneme_bin & (7 << 8)) % 2048
